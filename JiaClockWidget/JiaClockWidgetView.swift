...
struct JiaClockWidgetView: View {
    ...
    private var mediumLayout: some View {
        HStack(spacing: 14) {
            ...
            Spacer(minLength: 0)
            Text(entry.settings.effectiveSlogan).font(.footnote.weight(.medium)).foregroundStyle(theme.accent.opacity(0.92)).lineLimit(3).multilineTextAlignment(.trailing).frame(maxWidth: 120)
        }.padding(16)
    }
    ...
    private var largeLayout: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Spacer(minLength: 0)
            HStack { Spacer(); Text(String(localized: "widget.brand_footer")).font(.caption2).foregroundStyle(theme.tertiaryText) }
        }.padding(20)
    }
    ...
    private var brandMark: some View {
        HStack(spacing: 6) {
            ...
            Text(String(localized: "home.app_name")).font(.caption2.weight(.semibold)).foregroundStyle(theme.secondaryText)
        }
    }

    private var metaLines: some View {
        VStack(alignment: .leading, spacing: 4) {
            if entry.settings.showWeekday { Text(WidgetTimeFormatter.weekdayString(from: entry.date)).font(.subheadline.weight(.medium)).foregroundStyle(theme.secondaryText) }
            if entry.settings.showDate { Text(WidgetTimeFormatter.dateString(from: entry.date)).font(.subheadline).foregroundStyle(theme.tertiaryText) }
        }
    }
